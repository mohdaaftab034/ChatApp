...
export function BlockContactModal() {
  ...
  const canUnblock = Boolean(conversation?.canUnblock)
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (canUnblock) {
        ...
        updateConversation(conversation.id, { isBlocked: false, canUnblock: false })
        toast.success('Contact unblocked')
      } else {
        await blockUserApi(targetUser.id)
        updateConversation(conversation.id, { isBlocked: true, canUnblock: true })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={closeModal}>
      <div className="w-full max-w-md rounded-2xl border border-border bg-surface shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <div>
            <p className="text-xs uppercase tracking-wide text-text-secondary">{canUnblock ? 'Unblock contact' : 'Block contact'}</p>
            <h2 className="text-lg font-semibold text-foreground">{canUnblock ? 'Allow this person again?' : 'Block this contact?'}</h2>
          </div>
        ...
        <div className="space-y-4 px-5 py-5">
          ...
          <div className="flex items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-amber-700 dark:text-amber-300">
            ...
            <div className="space-y-1">
              <p className="text-sm font-medium">{canUnblock ? 'This contact is currently blocked.' : 'Blocking will hide their profile details and prevent new messages.'}</p>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-border px-5 py-4">
          ...
          >
            <Ban size={16} />
            {isSubmitting ? 'Working...' : (canUnblock ? 'Unblock contact' : 'Block contact')}
          </button>
        </div>
      </div>
    </div>
